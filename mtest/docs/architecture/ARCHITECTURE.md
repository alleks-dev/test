...
- app-facing facade для status/inspection/admin operations
- read-model publication і snapshot access
...
Application/runtime layer нормативно включає:
- `runtime facade`, який повертає snapshots або bounded query results, а не live mutable internals
- `read model coordinator`, який керує rebuild/publish flow для app-facing views
- `operation result store`, який відстежує `request_id`-based async operations і terminal statuses
...
Усі app-facing consumers:
- `admin_api`
- diagnostics/export views
- future bridge/inspection consumers

повинні залежати від вузького runtime facade, а не від concrete mutable runtime internals.

Facade:
- повертає snapshots, DTOs або bounded query results
- не розкриває lock ownership
- не повертає references на live mutable state

### 6.2. Read-model seams

Read-model layer повинен містити окремі ролі:
- `snapshot builder`
- `read model coordinator`
- facade-level published snapshot contracts

Ці seams є частиною application/runtime layer, а не domain core.

### 6.3. Async operation seam

Non-immediate runtime/admin operations повинні проходити через окремий async operation seam:
- `request_id`
- operation status
- terminal result/error
- bounded result storage/polling

Цей seam належить application/runtime layer і не повинен бути вбудований у hot-path routing/session/qos logic.

---

## 7. Внутрішня доменна модель
...
## 10. Що заборонено хардкодити під single broker
...
## 16. Єдина модель структури проєкту

Архітектурний документ фіксує **логічну модульну модель**, а `docs/architecture/TECH_STACK.md`
...
## 17. Definition of Done для архітектури
...
8. App-facing APIs не розкривають live mutable runtime state.
9. Async runtime/admin operations мають explicit request/result identity.
